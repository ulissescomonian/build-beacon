...
The monitoring token uses only Bitbucket Read permissions. The guided setup recommends all Read permissions currently offered by Bitbucket, including `read:pullrequest:bitbucket`, so read-only context such as pull-request authors is available without recreating the token later. It never requests Write, Admin, or Delete permissions. Action Mode requires a second token with exactly `read:user:bitbucket`, `read:pullrequest:bitbucket` and `write:pullrequest:bitbucket`, plus explicit allowlisting for the monitor and confirmation for every action.
